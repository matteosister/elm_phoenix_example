...
Elm.Book.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var switchOwned = function (owned) {
      return A2($Html.input,_U.list([$Html$Attributes.type$("checkbox"),$Html$Attributes.checked(owned)]),_U.list([$Html.text("")]));
   };
   ...
   var update = F2(function (action,model) {
      var _p0 = action;
      if (_p0.ctor === "Select") {
            var one = $Debug.log("Select");
            return _U.update(model,{owned: true});
         } else {
            return _U.update(model,{owned: false});
         }
   });
   var Unselect = {ctor: "Unselect"};
   var Select = {ctor: "Select"};
   var title = F2(function (address,title) {
      return A2($Html.button,_U.list([$Html$Attributes.$class("text-info"),A2($Html$Events.onClick,address,Select)]),_U.list([$Html.text(title)]));
   });
   var view = F2(function (address,model) {
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("book")]),
      _U.list([A2(title,address,model.title),pagesNumber(model.pages),switchOwned(model.owned)]));
   });
